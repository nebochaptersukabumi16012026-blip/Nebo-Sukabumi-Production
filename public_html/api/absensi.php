...
switch ($method) {
    ...
    case 'POST':
        if (!empty($data->anggota_id) && !empty($data->tanggal) && !empty($data->status)) {
            ...
            echo json_encode(array("status" => "success", "message" => "Absensi berhasil dicatat", "id" => $conn->lastInsertId()));
        } else {
    ...
    case 'PUT':
        if (!empty($data->id)) {
            ...
            echo json_encode(array("status" => "success", "message" => "Absensi berhasil diupdate"));
        } else {
    ...
    case 'DELETE':
        if (!empty($data->id)) {
            ...
            echo json_encode(array("status" => "success", "message" => "Absensi berhasil dihapus"));
        } else {
            echo json_encode(array("status" => "error", "message" => "ID tidak ditemukan"));
        }
        break;
