...
switch ($method) {
    ...
    case 'POST':
        if (!empty($data->judul) && !empty($data->tanggal)) {
            ...
            echo json_encode(["status" => "success", "message" => "Catatan berhasil ditambahkan", "id" => $conn->lastInsertId()]);
        } else {
    ...
    case 'PUT':
        if (!empty($data->id)) {
            ...
            echo json_encode(["status" => "success", "message" => "Catatan berhasil diupdate"]);
        } else {
