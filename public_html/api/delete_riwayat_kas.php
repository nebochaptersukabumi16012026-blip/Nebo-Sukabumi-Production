...
if (strpos($raw_id, 'kas_') === 0) {
    $extractedNum = intval(substr($raw_id, 4));
    $id = $extractedNum;
...
try {
    ...
    if ($id > 0 && $id_anggota <= 0) {
        $stmt_find = $conn->prepare("SELECT id_anggota FROM riwayat_kas WHERE id = ?");
        $stmt_find->execute([$id]);
        $row_rk = $stmt_find->fetch(PDO::FETCH_ASSOC);
        if ($row_rk) {
            $id_anggota = intval($row_rk['id_anggota']);
        } else {
    ...
    // A. HAPUS BARIS TRANSAKSI SPESIFIK DARI TABEL riwayat_kas & pembayaran
    if ($id > 0) {
        $stmt_del_rk = $conn->prepare("DELETE FROM riwayat_kas WHERE id = ?");
        $stmt_del_rk->execute([$id]);

        $stmt_del_pem = $conn->prepare("DELETE FROM pembayaran WHERE id = ?");
        $stmt_del_pem->execute([$id]);
    }

    // B. JIKA ID <= 0 DAN ID ANGGOTA VALID (HAPUS SEMUA RIWAYAT KAS ANGGOTA TERSEBUT)
    if ($id <= 0 && $id_anggota > 0) {
        $stmt_del_all_rk = $conn->prepare("DELETE FROM riwayat_kas WHERE id_anggota = ?");
        $stmt_del_all_rk->execute([$id_anggota]);
