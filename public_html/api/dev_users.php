...
if ($req_role !== 'DEVELOPER') {
    echo json_encode(array("status" => "error", "message" => "Akses ditolak: Hanya DEVELOPER"));
    exit();
}
...
if ($method == 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($anggota_id <= 0 || empty($username) || empty($password)) {
            echo json_encode(array("status" => "error", "message" => "Parameter tidak lengkap untuk membuat akun."));
            exit();
        }
        ...
        try {
            ...
            if ($stmt_check->fetch()) {
                echo json_encode(array("status" => "error", "message" => "Username sudah digunakan oleh akun lain."));
                exit();
            }
            ...
            $stmt_update->execute(array($username, $password, $role, $anggota_id));
            ...
            $stmt_insert->execute(array($username, $password, $role));
            
            error_log("DEBUG_CREATE_USER: Account successfully created for user $username");
            echo json_encode(array("status" => "success", "message" => "Akun berhasil dibuat."));
        } catch (PDOException $e) {
        ...
        if (empty($username) || empty($password)) {
            echo json_encode(array("status" => "error", "message" => "Username dan password baru wajib diisi."));
            exit();
        }
        ...
        try {
            ...
            $stmt_update->execute(array($password, $username));
            
            echo json_encode(array("status" => "success", "message" => "Password berhasil diperbarui."));
        } catch (PDOException $e) {
        ...
        echo json_encode(array("status" => "error", "message" => "Action tidak dikenal."));
        exit();
    }
}
