...
if ($method == 'GET' && $action == 'list') {
    ...
    if ($action == 'request') {
        ...
        if (empty($input)) {
            echo json_encode(["status" => "error", "message" => "NRA atau Username wajib diisi."]);
            exit();
        }
        ...
        try {
            ...
            if (!$row) {
                echo json_encode(["status" => "error", "message" => "NRA tidak ditemukan."]);
                exit();
            }
        ...
        try {
            ...
            if (!$request) {
                echo json_encode(["status" => "error", "message" => "Permintaan tidak ditemukan."]);
                exit();
            }
            ...
            try {
                ...
                if ($row) {
                    ...
                    if ($is_valid) {
                        ...
                        $stmt_up_ang->execute([':new_pass' => $hashed_password, ':id' => $id]);
                        ...
                        if (!empty($username)) {
                            ...
                            $stmt_up_usr->execute([':new_pass' => $hashed_password, ':username' => $username]);
                        }
                    ...
                    if ($row_usr) {
                        ...
                        if ($is_valid) {
                            ...
                            $stmt_up_usr->execute([':new_pass' => $hashed_password, ':id' => $id]);
                            ...
                            $stmt_up_ang->execute([':new_pass' => $hashed_password, ':username' => $username]);
