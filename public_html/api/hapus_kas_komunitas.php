<?php
// hapus_kas_komunitas.php - Khusus Detail Uang Kas Komunitas
...
if ($id <= 0) {
    ...
    echo json_encode(array(
        "status" => "error",
        "message" => "ID kas komunitas tidak valid"
    ));
    exit();
}
...
try {
    ...
    echo json_encode(array(
        "status" => "success",
        "message" => "Data kas komunitas berhasil dihapus",
    ...
    echo json_encode(array(
        "status" => "error",
        "message" => "Gagal menghapus kas komunitas: " . $e->getMessage()
    ));
}
?>
